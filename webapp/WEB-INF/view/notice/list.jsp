...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%> <%-- Alt+enter를 통해 jsp파일 한글깨짐 해결 --%>
...
<body>
			...
			<div class="search-form margin-top first align-right">
				...
				<form class="table-form">
					<fieldset>
						...
						<select name="f">
							<option ${(param.f == "title")?"selected":""} value="title">제목</option> <%-- 검색 값이 남아 있도록 함 --%>
							<option ${(param.f == "writer_id")?"selected":""}  value="writer_id">작성자</option>
						</select> 
						<label class="hidden">검색어</label>
						<input type="text" name="q" value="${param.q}"/> <%-- 검색 값이 남아 있도록 함 --%>
			...
			<div class="notice margin-top">
				...
				<table class="table">
					...
					<%-- <%
						...
						for(Notice n : list) {
							pageContext.setAttribute("n", n); // 지역변수 n 생성
					%> --%>
	...
	<c:set var="page" value="${(empty param.p) ? 1 : param.p}" /> <%-- c:set : 변수 생성 --%>
	<c:set var="startNum" value="${page - (page - 1) % 5}" />
	<c:set var="lastNum" value="${fn:substringBefore(Math.ceil(count / 10) , '.')}" /> <%-- substringBefore : 특정 문자 앞에서 끊는 함수 --%>
			
			<div class="indexer margin-top align-right">
				<h3 class="hidden">현재 페이지</h3>
				<div><span class="text-orange text-strong">${(empty param.p) ? 1 : param.p}</span> / ${lastNum} pages</div> <%-- empty : null이거나 빈문자열 이거나 --%>
			</div>
